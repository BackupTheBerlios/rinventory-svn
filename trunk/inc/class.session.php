...
class Session
{
   ...
   function startSession(){
      ...
      /* Update users last active timestamp */
         ...
         $login->addActiveUser($this->username, $this->time);
      }
      
      /* Remove inactive visitors from database */
      $login->removeInactiveUsers();
      $login->removeInactiveGuests();
   ...
   function checkLogin(){
      ...
      if(isset($_SESSION['username']) && isset($_SESSION['userid']) && $_SESSION['username'] != GUEST_NAME){
         ...
         if($login->confirmUserID($_SESSION['username'], $_SESSION['userid']) != 0){
         ...
         $this->userinfo  = $login->getUserInfo($_SESSION['username']);
         $this->username  = $this->userinfo['username'];
         $this->userid    = $this->userinfo['userid'];
         $this->userlevel = $this->userinfo['level'];
         $this->uniqueid    = $this->userinfo['id'];
         return true;
      }
      /* User not logged in */
      else{
   ...
   function login($subuser, $subpass, $subremember){
      ...
      $result = $login->confirmUserPass($subuser, $subpass);
      ...
      $this->userinfo  = $login->getUserInfo($subuser);
      $this->username  = $_SESSION['username'] = $this->userinfo['username'];
      $this->userid    = $_SESSION['userid']   = $this->generateRandID();
      $this->userlevel = $this->userinfo['level'];
      
      /* Insert userid into database and update active users table */
      $login->updateUserField($this->username, "userid", $this->userid);
      $login->addActiveUser($this->username, $this->time);
      $login->removeActiveGuest($_SERVER['REMOTE_ADDR']);
